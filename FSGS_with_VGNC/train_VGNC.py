...
def training(dataset, opt, pipe, args):
    testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, loss_coef, exp_number = args.test_iterations, \
            args.save_iterations, args.checkpoint_iterations, args.start_checkpoint, args.debug_from, args.loss_coef, args.exp_number
    first_iter = 0
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        with torch.no_grad():
            ...
            if  iteration < opt.densify_until_iter:
                ...
                current_count = scene.gaussians.get_xyz.shape[0]

                if iteration > opt.densify_from_iter and iteration % opt.densification_interval == 0 and iteration <= 5000:
                    size_threshold = None
                    gaussians.densify_and_prune(opt.densify_grad_threshold, opt.prune_threshold, scene.cameras_extent, size_threshold, iteration)

                if iteration > 5000 and iteration % opt.densification_interval == 0 and current_count != exp_number:
                    size_threshold = None
                    if current_count > exp_number:
                        extra_prune -= 0.05
                        print("\n[ITER {}] Extra prune {}".format(iteration, extra_prune)) 
                    gaussians.densify_and_prune_2(opt.densify_grad_threshold, opt.prune_threshold, scene.cameras_extent, size_threshold, iteration, exp_number, extra_prune)
            ...
            # iteration % 100 == 0
            # Of course you can choose iteration % 50 == 0
            if iteration % 100 == 0 and 500 < iteration <=5000 and len(scene.getValCameras()) != 0:
                ...
                for viewpoint in scene.getValCameras():
                    ...
                    val_l1_loss += l1_loss(rendered_image, gt_image).mean().item()
                    val_psnr += psnr(rendered_image, gt_image).mean().item()

                # Compute average L1 loss and PSNR for validation set
                val_l1_loss /= len(scene.getValCameras())
                val_psnr /= len(scene.getValCameras())
                
                if val_psnr > best_psnr:
                    best_psnr = val_psnr
                    best_expnum = scene.gaussians.get_xyz.shape[0]
                    back_num = 0
                    
                if val_psnr < best_psnr:
                    back_num += 1

                if (back_num == 0 or back_num >= 5) and exp_number != best_expnum:
                    exp_number = best_expnum
                    print("\n[ITER {}] Change Expect Number to: {}".format(iteration, exp_number))
                
                if tb_writer:
                    tb_writer.add_scalar('validation/l1_loss', val_l1_loss, iteration)
                    tb_writer.add_scalar('validation/psnr', val_psnr, iteration)
                    tb_writer.add_scalar('Exp_number', exp_number, iteration)
...
def training_report(tb_writer, iteration, Ll1, loss, l1_loss, elapsed, testing_iterations, scene : Scene, renderFunc, renderArgs):
    if tb_writer:
        ...
        tb_writer.add_scalar('GPU Memory(GB)', torch.cuda.memory_allocated() / (1024**3), iteration)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--exp_number", type=int, default = 200_000)
