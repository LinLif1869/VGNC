...
class GaussianModel:
    ...
    # Our "densify_and_prune_2" method adds the exp_number parameter to control the maximum number of Gaussian spheres
    def densify_and_prune_2(self, max_grad, min_opacity, extent, max_screen_size, iter, exp_number, extra_prune = 0):
        # Check if the number of Gaussian spheres has reached the threshold
        if self._xyz.shape[0] == exp_number:
            return
        grads = self.xyz_gradient_accum / self.denom
        grads[grads.isnan()] = 0.0
        ...
        if self._xyz.shape[0] > exp_number:
            ...
            num_to_prune = self._xyz.shape[0] - exp_number + int(exp_number * extra_prune)
            # Randomly generate a mask to select points to prune
            prune_mask = torch.zeros(num_points, dtype=torch.bool, device="cuda")

            prune_indices = torch.randperm(num_points)[:num_to_prune]
