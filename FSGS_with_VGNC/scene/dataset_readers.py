...
def readColmapSceneInfo(path, images, eval=False, n_views=0, llffhold=8, val=False):
    # Determine train/val directory
    # Must change here for your own dataset.
    train_val_dir = "train_val"
    
    if val:
        ply_path = os.path.join(path, train_val_dir, "sparse/0/points3D.ply")
        bin_path = os.path.join(path, train_val_dir, "sparse/0/points3D.bin")
        # ply_path = os.path.join(path, train_val_dir, "dense/fused.ply")
    else:
        ply_path = os.path.join(path, str(n_views) + "_views/dense/sparse/points3D.ply")
    ...
    # cam_infos = [i for i in cam_infos if not i.image_name.startswith("frame")]
    ...
    #     if not eval:
    #         for i in cam_infos:
    #             if i.image_name not in [j.image_name for j in train_cam_infos]:
    #                 test_cam_infos.append(i)
    #     assert len(train_cam_infos) == n_views
         
    old_cam_infos = [i for i in cam_infos if not i.image_name.startswith("frame")]
    val_cam_infos = [i for i in cam_infos if i.image_name.startswith("frame")]

    train_cams = [f for f in sorted(glob.glob(os.path.join(path, train_val_dir, 'images', '*')))] 
    train_cams_2 = [os.path.basename(path) for path in train_cams if not os.path.basename(path).startswith("frame")]
    filtered_train_cams = [os.path.splitext(path)[0] for path in train_cams_2]
    # print(filtered_train_cams)
    train_cam_infos = [i for i in cam_infos if i.image_name in filtered_train_cams]
    test_cam_infos_2 = []
    for i in old_cam_infos:
        if i.image_name not in [j.image_name for j in train_cam_infos]:
            test_cam_infos_2.append(i)
            if eval:
                test_cam_infos = [c for idx, c in enumerate(test_cam_infos_2) if idx % llffhold == 0]
            else:
                test_cam_infos = test_cam_infos_2
                
    # test_cam_infos = [i for (i in old_cam_infos and i not in train_cam_infos)]
