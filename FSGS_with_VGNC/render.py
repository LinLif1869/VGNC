...
def render_set(model_path, name, iteration, views, gaussians, pipeline, background, args):
    ...
    with open(os.path.join(args.model_path, 'render_time.txt'), 'a') as f:
        f.write(f"{name} Average rendering time: {avg_time:.5f} seconds\n")
...
def render_video(source_path, model_path, iteration, views, gaussians, pipeline, background, fps=30):
    ...
    final_video = cv2.VideoWriter(os.path.join(render_path, 'final_video.avi'), fourcc, fps, size)
...
def render_sets(dataset : ModelParams, pipeline : PipelineParams, args):
    ...
    with torch.no_grad():
        ...
        render_set(dataset.model_path, "val", scene.loaded_iter, scene.getValCameras(), gaussians, pipeline, background, args)
